...
在这小节以前,我们讨论的都是如何匹配文本,但是某些时候我们不仅需要对文本进行匹配处理,还需要修改文本使它符合我们的需求,这时我们可以使用`JavaScript`的`string.replace(pattern/string,string)`函数实现替换功能.

例如我们要匹配一个商店物品的价格,这个价格通常而言小数点后跟两位数,但是如果第三位不为`0`,我们也要匹配它,例如`3.111111`需要替换成`3.111`,而`3.110000`则需要替换成`3.11`, 最后`3.1`的情况先不考虑,首先我们需要实现一个小数部分的匹配`/\.\d\d+/`,需要注意的是我们需要捕获小数点后的前三位数字,因此这种匹配表达式不符合我们的要求,我们需要匹配至少两位数字,第三位数字如果为`0`则不匹配,否则也需要匹配,因此表达式变成了`/\.\d\d[1-9]?/`,我们需要把匹配的小数部分替换掉原来的小数部分,所以接下来考虑增加捕获组`/(\.\d\d[1-9]?)\d*/`,此时捕获部分`(\.\d\d[1-9]?)`可以匹配至少两个数字,如果第三个数字不是`0`,则也需要捕获第三个数字,否则第三个数字就不进行捕获处理.需要注意的是总体匹配的小数部分可能不止三位数字,所以为了匹配更多的小数部分则需要加上`\d*`. 当捕获到小数点后三位数字后将它替换原来的小数部分,这样就符合我们的需求了.

``` javascript
var pattern = /(\.\d\d[1-9]?)\d*/,
    num = 3.111111,
    num2 = 3.11000000000,
    num3 = 3.11;

var result = num.toString().replace(pattern,"$1");  
console.log(result);    //3.111

var result = num2.toString().replace(pattern,"$1"); 
console.log(result);    //3.11

var result = num3.toString().replace(pattern,"$1"); 
console.log(result);    //3.11
```